-- PG port of 00092_alert_inbox.sql | status: NEEDS-REVIEW | flags: HAS-BACKTICK,HAS-DQUOTE | reviewed: YES
-- +goose Up
...
INSERT INTO permissions (name, description) VALUES
  ...
  ('driver:write-self', 'Driver self-service: request advances, submit expenses') ON CONFLICT DO NOTHING;
INSERT INTO role_permissions (role_id, permission_id)
SELECT 1, id FROM permissions WHERE name IN (
  'alerts:read','alerts:write','kharcha:approve','compliance:read',
  'driver:read-self','driver:write-self') ON CONFLICT DO NOTHING;
INSERT INTO role_permissions (role_id, permission_id)
SELECT 2, id FROM permissions WHERE name IN ('alerts:read','alerts:write','compliance:read') ON CONFLICT DO NOTHING;
INSERT INTO role_permissions (role_id, permission_id)
