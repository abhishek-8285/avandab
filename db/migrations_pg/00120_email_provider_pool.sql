-- PG port of 00120_email_provider_pool.sql | status: PORTABLE | flags: none
-- +goose Up
...
--    (created_at)::date / to_char(created_at, 'YYYY-MM') so resets are automatic.
...
CREATE TABLE IF NOT EXISTS email_provider_counters (
    ...
    current_day   TEXT NOT NULL DEFAULT (CURRENT_DATE),
    current_month TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP, 'YYYY-MM')),
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT (CURRENT_TIMESTAMP),
...
INSERT INTO email_providers (provider, enabled, priority, daily_quota, monthly_quota, cost_per_1k, host, port, from_addr) VALUES
    ...
    ('primary',1, 10, 0, 0, 0, '', '', 'billing@avandab.com') ON CONFLICT DO NOTHING;
INSERT INTO email_provider_counters (provider, daily_used, monthly_used, current_day, current_month) VALUES
    ('brevo',  0, 0, CURRENT_DATE, to_char(CURRENT_TIMESTAMP, 'YYYY-MM')),
    ('resend', 0, 0, CURRENT_DATE, to_char(CURRENT_TIMESTAMP, 'YYYY-MM')),
    ('direct', 0, 0, CURRENT_DATE, to_char(CURRENT_TIMESTAMP, 'YYYY-MM')),
    ('primary',0, 0, CURRENT_DATE, to_char(CURRENT_TIMESTAMP, 'YYYY-MM')) ON CONFLICT DO NOTHING;
-- +goose Down
