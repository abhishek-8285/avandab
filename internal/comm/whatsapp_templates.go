...
func FormatTripDispatchMessage(baseURL, origin, destination, vehicleID string) string {
	if origin == "" {
		origin = "Origin"
	}
	if destination == "" {
	...
	return fmt.Sprintf("🚚 Avandab Trip Dispatched: %s ➔ %s | Live Tracking: %s/tracking#v=%s",
		origin, destination, baseURL, vehicleID)
}
...
func FormatTripTrackingMessage(baseURL, tripID, tripNumber, origin, destination string) string {
	trackingURL := fmt.Sprintf("%s/customer/tracking/%s", baseURL, tripID)
...
func FormatBookingConfirmedMessage(baseURL, bookingNumber, origin, destination, vehicleOrURL string) string {
	trackingURL := vehicleOrURL
	if !strings.HasPrefix(trackingURL, "http://") && !strings.HasPrefix(trackingURL, "https://") {
		if trackingURL != "" {
			trackingURL = fmt.Sprintf("%s/tracking#v=%s", baseURL, trackingURL)
		} else {
			trackingURL = fmt.Sprintf("%s/tracking#b=%s", baseURL, bookingNumber)
		}
	}
	if origin != "" && destination != "" {
