		...
		if !expiry.when.IsZero() && expiry.when.Before(now) {
			...
			if cmd.OverrideMaintenance && len(strings.TrimSpace(cmd.OverrideReason)) >= 10 {
				...
				if dbGetter, ok := ctx.Repositories().AuditLogs().(repository.DBGetter); ok && dbGetter.DB() != nil {
					...
					if _, ierr := repository.ExecTx(ctx, dbGetter.DB(), `INSERT INTO dispatch_overrides (id, tenant_id, trip_id, vehicle_id, driver_id, blocked_by, reason, overridden_by, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
			...
			if !isExempt(ctx, "vehicle", cmd.VehicleID, "puc") {
				if cmd.OverrideMaintenance && len(strings.TrimSpace(cmd.OverrideReason)) >= 10 {
					...
					if dbGetter, ok := ctx.Repositories().AuditLogs().(repository.DBGetter); ok && dbGetter.DB() != nil {
						...
						_, _ = repository.ExecTx(ctx, dbGetter.DB(), `CREATE TABLE IF NOT EXISTS dispatch_overrides (
						...
						_, _ = repository.ExecTx(ctx, dbGetter.DB(), `INSERT INTO dispatch_overrides (id, tenant_id, trip_id, vehicle_id, driver_id, blocked_by, reason, overridden_by, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
...
func getPUCExpiry(ctx ports.TxContext, vehicleID string) *time.Time {
	...
	var t sql.NullTime
	_ = repository.QueryRowTx(ctx, dbGetter.DB(), `SELECT puc_expiry FROM vehicles WHERE id = $1`, vehicleID).Scan(&t)
