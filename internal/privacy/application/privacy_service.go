...
// NewPrivacyService wires the ledger over the caller's DB.
func NewPrivacyService(db *sql.DB, idGen ports.IDGenerator) *PrivacyService {
	return &PrivacyService{db: db, idGen: idGen}
}
...
func (s *PrivacyService) GetBreach(ctx context.Context, tenantID, id string) (*BreachIncident, error) {
	...
	err := scanBreach(repository.QueryRowTx(ctx, s.db,
