...
type BreachWatchService struct {
	...
	alerts  *legacy.OpsAlertService
	log     *slog.Logger
}

// NewBreachWatchService wires the watch over the caller's DB. Nil deps are
// legal at construction; SweepOverdue fails fast if any is missing.
func NewBreachWatchService(db *sql.DB, privacy *PrivacyService, alerts *legacy.OpsAlertService, log *slog.Logger) *BreachWatchService {
	if log == nil {
		log = slog.Default()
	}
	return &BreachWatchService{db: db, privacy: privacy, alerts: alerts, log: log}
}
...
func (s *BreachWatchService) SweepTenant(ctx context.Context, tenantID string) (int, error) {
	...
	for _, b := range overdue {
		...
		_, err = s.alerts.CreateAlert(ctx, legacy.OpsAlert{
			TenantID:  tenantID,
			AlertType: legacy.OpsAlertComplianceBreach,
			Severity:  legacy.OpsAlertSeverityCritical,
			...
			EntityType: legacy.StrPtr(breachWatchEntity),
			EntityID:   legacy.StrPtr(b.ID),
		})
		if err != nil {
...
func (s *BreachWatchService) alreadyAlerted(ctx context.Context, tenantID, breachID string) (bool, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		tenantID, legacy.OpsAlertComplianceBreach, breachWatchEntity, breachID).Scan(&n)
	return n > 0, err
}
