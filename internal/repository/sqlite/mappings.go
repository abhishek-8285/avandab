...
func toCreateUserRowWithRole(u db.CreateUserRow, role domain.Role) domain.User {
	return domain.User{
		...
		EmailVerifiedAt: fromNullTime(u.EmailVerifiedAt),
...
func toDomainVehicle(v db.Vehicle) domain.Vehicle {
	return domain.Vehicle{
		...
		InsuranceExpiry:    v.InsuranceExpiry.Time,
		FitnessExpiry:      v.FitnessExpiry.Time,
		PermitExpiry:       v.PermitExpiry.Time,
		RCExpiry:           v.RcExpiry.Time,
		PUCExpiry:          fromNullTime(v.PucExpiry),
		Status:             domain.VehicleStatus(v.Status),
		Blocked:            int64ToBool(v.Blocked),
...
func toDomainCompanySetting(c db.CompanySetting) domain.CompanySettings {
	return domain.CompanySettings{
		...
		PanNumber:     fromNullString(c.PanNumber),
...
func auditLogRowToWithUser(
	...
	ipAddress sql.NullString, location sql.NullString, createdAt time.Time, userName sql.NullString,
