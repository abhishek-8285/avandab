...
func (r *SQLRepository) CreateVehicle(ctx context.Context, vehicle domain.Vehicle) (domain.Vehicle, error) {
	created, err := r.Q(ctx).CreateVehicle(ctx, db.CreateVehicleParams{
		...
		InsuranceExpiry:    sql.NullTime{Time: vehicle.InsuranceExpiry, Valid: !vehicle.InsuranceExpiry.IsZero()},
		FitnessExpiry:      sql.NullTime{Time: vehicle.FitnessExpiry, Valid: !vehicle.FitnessExpiry.IsZero()},
...
func (r *SQLRepository) UpdateVehicle(ctx context.Context, vehicle domain.Vehicle) (domain.Vehicle, error) {
	...
	_, err := r.exec(ctx, `UPDATE vehicles SET registration_number = $1, vehicle_number = $2,
		...
		vehicle.Capacity, string(vehicle.FuelType), sql.NullTime{Time: vehicle.InsuranceExpiry, Valid: !vehicle.InsuranceExpiry.IsZero()},
		sql.NullTime{Time: vehicle.FitnessExpiry, Valid: !vehicle.FitnessExpiry.IsZero()}, sql.NullTime{Time: vehicle.PermitExpiry, Valid: !vehicle.PermitExpiry.IsZero()}, string(vehicle.Status),
