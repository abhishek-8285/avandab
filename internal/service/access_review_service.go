...
func (s *AccessReviewService) OpenDueReview(ctx context.Context, tenantID, userID, roleName, period string, dueAt time.Time) (string, error) {
	db := s.consentDB()
	if db == nil {
		return "", errors.New("access review unavailable: storage does not support raw DB access")
	}
	if userID == "" {
	...
	var got string
	if err := repository.QueryRowTx(ctx, db, `
...
func (s *AccessReviewService) GetAccessReview(ctx context.Context, tenantID, id string) (*AccessReview, error) {
	db := s.consentDB()
	if db == nil {
		return nil, errors.New("access review unavailable: storage does not support raw DB access")
	}
	var r AccessReview
	err := scanAccessReview(repository.QueryRowTx(ctx, db,
