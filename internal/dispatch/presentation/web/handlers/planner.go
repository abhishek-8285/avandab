...
import (
	...
	legacyhandlers "transport-app/internal/handlers"
...
type DispatchHandlers struct {
	App     *legacyhandlers.App
	Planner *dispatchapp.PlannerService
	Tuner   *dispatchapp.TunerService
}

func (h *DispatchHandlers) Routes(r chi.Router) {
	r.With(middleware.ResourcePermission(h.App.AuthSrv, "dispatch", "read")).
		Get("/", h.ListRuns)
	r.With(middleware.ResourcePermission(h.App.AuthSrv, "dispatch", "create")).
		Get("/new", h.NewRun)
	r.With(middleware.ResourcePermission(h.App.AuthSrv, "dispatch", "create")).
		Post("/runs", h.CreateRun)
	r.With(middleware.ResourcePermission(h.App.AuthSrv, "dispatch", "read")).
		Get("/runs/{runID}", h.ViewRun)
	r.With(middleware.ResourcePermission(h.App.AuthSrv, "dispatch", "update")).
...
func (h *DispatchHandlers) ListRuns(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		legacyhandlers.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to list runs"})
		return
	}
	if legacyhandlers.IsDatastarRequest(r) || strings.HasPrefix(r.URL.Path, "/api/") {
		legacyhandlers.WriteJSON(w, http.StatusOK, map[string]interface{}{"runs": runs})
		return
	}
	session := legacyhandlers.SessionOf(h.App, r)
	h.App.RenderPage(w, r, "dispatch_runs.html", legacyhandlers.PageData{
...
func (h *DispatchHandlers) NewRun(w http.ResponseWriter, r *http.Request) {
	session := legacyhandlers.SessionOf(h.App, r)
	h.App.RenderPage(w, r, "dispatch_run_new.html", legacyhandlers.PageData{
...
func (h *DispatchHandlers) CreateRun(w http.ResponseWriter, r *http.Request) {
	tenantID := shared.TenantIDFromContext(r.Context())
	user := legacyhandlers.SessionOf(h.App, r)
	actorID := ""
	if user != nil {
		actorID = user.UserID
	}
	...
	if strings.HasPrefix(r.URL.Path, "/api/") || r.Header.Get("Content-Type") == "application/json" {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			legacyhandlers.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid JSON: " + err.Error()})
			return
		}
	...
	if err != nil {
		...
		legacyhandlers.WriteJSON(w, http.StatusUnprocessableEntity, map[string]string{"error": rootMsg(err)})
		return
	}
	legacyhandlers.WriteAuditLog(r, h.App.DB, "dispatch.run_create", "planner_runs", runID, map[string]any{
		"source": cmd.Source, "stops": len(cmd.Stops),
	})
	if strings.HasPrefix(r.URL.Path, "/api/") {
		legacyhandlers.WriteJSON(w, http.StatusCreated, map[string]string{"run_id": runID})
		return
	}
...
func (h *DispatchHandlers) ViewRun(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		legacyhandlers.WriteJSON(w, http.StatusNotFound, map[string]string{"error": "run not found"})
		return
	}
	if strings.HasPrefix(r.URL.Path, "/api/") {
		legacyhandlers.WriteJSON(w, http.StatusOK, detail)
		return
	}
	session := legacyhandlers.SessionOf(h.App, r)
	h.App.RenderPage(w, r, "dispatch_run_detail.html", legacyhandlers.PageData{
...
func (h *DispatchHandlers) TuneRun(w http.ResponseWriter, r *http.Request) {
	...
	legacyhandlers.WriteAuditLog(r, h.App.DB, "dispatch.run_tune", "planner_runs", runID, map[string]any{
		"op": op.Op, "stop_id": op.StopID, "route_id": op.RouteID,
	})
	if r.Method == http.MethodPatch || strings.HasPrefix(r.URL.Path, "/api/") {
		legacyhandlers.WriteJSON(w, http.StatusOK, map[string]interface{}{"run_id": runID, "kpi": kpi})
		return
	}
	target := legacyhandlers.SafeRedirect(r, "/dispatch/runs/"+runID, "/dispatch")
...
func (h *DispatchHandlers) PlanRun(w http.ResponseWriter, r *http.Request) {
	...
	legacyhandlers.WriteAuditLog(r, h.App.DB, "dispatch.run_plan", "planner_runs", runID, map[string]any{
