...
export function StateView({ state, title, message, icon, onRetry }: { state: State; title?: string; message?: string; icon?: string; onRetry?: () => void }) {
  if (state === 'loading') return (<View style={styles.center} accessibilityLiveRegion="polite"><ActivityIndicator color={Colors.primary} accessibilityLabel="Loading…" /><Text style={styles.loadingText}>Loading…</Text></View>);
  ...
  const cfg = map[state];
  return (<Card style={styles.card}><View style={styles.iconBox} accessible={false}><MaterialCommunityIcons name={(icon as any) || cfg.defaultIcon} size={28} color={Colors.textMuted} /></View><Text style={styles.title} accessibilityRole="header">{title || cfg.defaultTitle}</Text>{message ? <Text style={styles.message} accessibilityLiveRegion="polite">{message}</Text> : null}{onRetry ? (<TouchableOpacity style={styles.retryBtn} onPress={onRetry} accessibilityRole="button" accessibilityLabel="Retry"><Text style={styles.retryText}>Retry</Text></TouchableOpacity>) : null}</Card>);
}
